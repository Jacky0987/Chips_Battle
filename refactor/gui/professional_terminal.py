...
class ProfessionalTerminal:
    ...
    def create_status_bar(self):
        """📟 创建状态栏"""
        status_frame = tk.Frame(self.main_container, bg=self.colors['bg_secondary'], height=25)
        ...
        # 中间：网络延迟
        self.latency_label = tk.Label(status_frame,
                                     font=self.fonts['mono_small'],
                                     fg=self.colors['positive'],
                                     bg=self.colors['bg_secondary'],
                                     text="PING: <1ms")
        self.latency_label.pack(side=tk.LEFT, padx=20, pady=4)
