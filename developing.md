...
阶段二：家园体系建设（2-3周）
- 设计并实现 PersonalAsset 模型（/models/home/personal_asset.py）
- 完成房地产与奢侈品市场数据定义（/data/definitions/personal_assets.json）
- 实现 HomeService 按时间与宏观事件更新资产价值
- 完成 home 命令集：market/buy/assets/sell

阶段三：企业帝国基础（4-6周）
- 搭建公司核心模型与服务（Company/Technology/Production/Inventory）
- 建立工业与科技树数据（industries.json/technologies.json）
- 实现公司命令集与生产闭环（design/build/produce/sell）
- 设计并实现 IPO 上市流程（company ipo）
...
3. 数据访问层 (DAL):
  ...
  - 工作单元 (/dal/unit_of_work.py): 实现 SqlAlchemyUnitOfWork，封装数据库会话的生命周期。
...
第二章：神谕与身份 (The Oracle & Identity)
【进度状态：完成度 80%】
目标: 构建玩家与游戏世界交互的桥梁——命令系统，并建立一个健壮、可扩展的权限与角色系统。
技术实现要点:
1. 命令处理系统:
