...
class MarketCommand(Command):
    ...
    def execute(self, args: list, context: Dict[str, Any]) -> str:
        if not args:
            return self._show_market_home(context)
        
        subcommand = args[0].lower()
        
        if subcommand == 'app':
            if len(args) < 2:
                return "用法: market app <应用名称>"
            app_name = args[1]
            return self._show_app_detail(app_name, context)
        elif subcommand == 'category':
            if len(args) < 2:
                return self._show_categories()
            category = args[1]
            return self._show_category_apps(category)
        elif subcommand == 'search':
            if len(args) < 2:
                return "用法: market search <关键词>"
            keyword = ' '.join(args[1:])
            return self._search_apps(keyword)
        elif subcommand == 'buy':
            if len(args) < 2:
                return "用法: market buy <应用名称>"
            app_name = args[1]
            return self._buy_app(app_name, context)
        else:
            return "未知的子命令。可用命令: app, category, search, buy"
    
    def _show_market_home(self, context: Dict[str, Any]) -> str:
        """显示应用市场主页"""
        apps = self.app_service.get_available_apps()
        user_id = context.get('user_id')
        owned_apps = self.app_service.get_user_owned_apps(user_id) if user_id else []
    ...
    def _show_app_detail(self, app_name: str, context: Dict[str, Any]) -> str:
        """显示应用详情"""
    ...
    def _buy_app(self, app_name: str, context: Dict[str, Any]) -> str:
        """购买应用"""
        user_id = context.get('user_id')
        if not user_id:
            return "❌ 请先登录"
        
        result = self.app_service.purchase_app(user_id, app_name)
